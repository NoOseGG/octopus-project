...
const SubjectConstituentDoc: React.FC<MyComponentProps> = ({ constituent_doc }) => {
  return (
    <Card title="Сведения об изменениях, внесенных в учредительный документ" style={{ width: '100%' }}>
      ...
      {constituent_doc[0].from_dttm && (
        <DataField name="Дата события" content={formatDate(constituent_doc[0].from_dttm)} />
      )}
      {constituent_doc[0].to_dttm && (
        <DataField name="Дата отмены события" content={formatDate(constituent_doc[0].to_dttm)} />
      )}
      {constituent_doc[0].doc_dttm && (
        <DataField name="Дата подачи документов" content={formatDate(constituent_doc[0].doc_dttm)} />
      )}
