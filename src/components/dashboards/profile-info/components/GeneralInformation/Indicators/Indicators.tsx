        ...
        {metric_address_2[0]?.risk_level && (
          <MeterGaugePlot
            ...
            description={
              <Description>
                <Paragraph>
                  <Bold>Индикатор частоты смены юридического адреса</Bold> — это количественный показатель,
                  предназначенный для мониторинга стабильности и надежности компании. Он отслеживает количество
                  изменений юридического адреса организации за определенный период времени. Этот индикатор может служить
                  важным сигналом для инвесторов и партнеров, поскольку частые изменения адреса могут указывать на
                  потенциальные управленческие проблемы или нестабильность внешней бизнес-среды.
                </Paragraph>
                <Paragraph>
                  Для наглядности и удобства интерпретации результаты представлены в виде цветовой кодировки:
                </Paragraph>
                <Paragraph>
                  <Green>Зеленый:</Green> Обозначает редкую смену адреса, что может свидетельствовать о низком уровне
                  риска.
                </Paragraph>
                <Paragraph>
                  <Orange>Оранжевый:</Orange> Указывает на среднюю частоту смены адреса, что требует более глубокого
                  анализа для оценки потенциальных рисков.
                </Paragraph>
                <Paragraph>
                  <Red>Красный:</Red> Сигнализирует о частой смене адреса, что может быть признаком высокого уровня
                  риска и нестабильности.
                </Paragraph>
                <Paragraph>
                  Значение индекса, отображаемое на индикаторе, представляет собой агрегированное число смены
                  юридического адреса компанией за весь период функционирования компании.
                </Paragraph>
                <Paragraph>
                  Этот индикатор является частью комплексной системы оценки рисков и может использоваться в сочетании с
                  другими метриками для получения более полной картины финансового здоровья и устойчивости компании.
                </Paragraph>
              </Description>
            }
          />
