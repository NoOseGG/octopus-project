...
export const doCalculateBankruptedPercent = createAsyncThunk<ResponsePercent, RequestData>(
  'doCalculateBankruptedPercent',
  async ({ filters }) => {
    try {
      const year = getCurrentYear();
      const url = constructorUrlForDashboard(
        DASH.BASE + DASH.LEGAL_ENTITY + DASH.STATUS_AT + DASH.DATE_AFTER_LIQUIDATED(`${year}-01-01`),
        filters,
        true,
        false,
      );
